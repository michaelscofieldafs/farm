...
export default function TicTacToeOnChain() {
    ...
    const isPlayMoveRef = useRef(false);

    const isFetchingGame = useRef(false);
    ...
    // GAME SOUNDS
    // start sound
    const [playMove] = useSound('/sounds/game-move.mp3', { volume: 0.5 });
    // Win sound
    const [playWin] = useSound('/sounds/game-winner.mp3', { volume: 0.5 });
    // Loss sound
    const [playLoss] = useSound('/sounds/game-over.mp3', { volume: 0.5 });
...
`}
                                ...
                                {statusType === PlayerGameStatus.None &&
                                    <div className="mt-2 p-4 rounded-2xl border border-slate-700 bg-[linear-gradient(180deg,rgba(255,255,255,0.02),transparent)]">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            <input
                                                className="p-2 rounded-md border border-slate-600 bg-slate-900 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-[80px]"
                                                placeholder="Stake"
                                                value={stakeInput}
                                                onChange={(e) => {
        ...
        ${!isLoadingCreateGame
                                                        ? "bg-primary border border-primary rounded-lg font-medium hover:bg-transparent hover:text-primary text-darkmode"
    ...
    `}
                    ...
                    {/**
                    {statusType !== null &&
                        <footer className="mt-6 text-center text-xs text-slate-400 px-2 md:px-0">Developed with 💜 by SavvyGirl <br /> Tic-Tac-Toe is part of SavvyGirl products.</footer>
                    }
                         */}
                </div>
